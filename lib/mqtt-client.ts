...
// 서버 사이드 렌더링을 위한 더미 MQTT 클라이언트
class DummyMQTTClient implements IMQTTClient {
  constructor(options?: any) {
    console.log("더미 MQTT 클라이언트가 생성되었습니다. 서버 사이드 렌더링 중입니다.");
  }
...
let RealMQTTClient: any = DummyMQTTClient;
...
// 팩토리 함수: 서버 사이드에서는 더미 클라이언트, 클라이언트 사이드에서는 실제 클라이언트 반환
const createMqttClient = (options?: any): IMQTTClient => {
  if (typeof window === 'undefined') {
    return new DummyMQTTClient(options);
  }
